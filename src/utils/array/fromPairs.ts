...
// 辅助函数：检查是否为可迭代的元组
function isIterableTuple(value: any): value is Iterable<unknown> {
 ...
 * 增强版的 fromPairs 函数
 */
export function fromPairs<T = any>(
  pairs:
    | Array<[PropertyKey, T]>
    | Iterable<[PropertyKey, T]>
    | null
    | undefined,
  options: FromPairsOptions = {},
): Record<PropertyKey, T> {
  // 处理 null 或 undefined 输入
  if (pairs === null) {
    return {} as Record<PropertyKey, T>;
  }
  ...
  for (const pair of pairs) {
    ...
    if (pair && (Array.isArray(pair) || isIterableTuple(pair))) {
      let [key, value] = pair as [PropertyKey, T];

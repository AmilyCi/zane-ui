import { isFirefox } from "src/utils/is/isFirefox";
import type { MentionCtx, MentionOption } from "./types";
import { castArray } from "src/utils";

export const filterOption = (pattern: string, option: MentionOption) => {
  const lowerCase = pattern.toLocaleLowerCase();
  const label = option.label || option.value || '';
  return label.toLocaleLowerCase().includes(lowerCase);
}
...
export const getCursorPosition = (
  ...
  if (options.debug) {
    const el = document.querySelector(
      '#input-textarea-caret-position-mirror-dev'
    );
    el?.remove();
  }

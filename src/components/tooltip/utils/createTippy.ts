...
export function createTippy(
  ...
  // 🔒 Private methods
  // ===========================================================================
  function getNormalizedTouchSettings(): [boolean | string, number] {
  ...
  function getIsDefaultRenderFn(): boolean {
    return !!(instance.props.render as any)?.$$tippy;
  }
  ...
  function removeListeners(): void {
    listeners.forEach(
      ({ eventType, handler, node, options }: ListenerObject) => {
  ...
  function onBlurOrFocusOut(event: FocusEvent): void {
    if (
      !instance.props.trigger.includes('focusin') &&
  ...
  function createPopperInstance(): void {
    ...
    type ExtendedModifiers = Partial<TippyModifier> | StrictModifiers;

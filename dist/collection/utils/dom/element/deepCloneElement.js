export function deepCloneElement(element, options = {}) {
    const { cloneAttributes = true, cloneEvents = true, cloneStyles = true, deepCloneChildren = true, } = options;
    // 1. 克隆元素本身（包括属性和子元素）
    const clone = element.cloneNode(deepCloneChildren);
    // 2. 克隆样式
    if (cloneStyles) {
        // 2.1. 克隆内联样式
    ...
    if (deepCloneChildren && element.children.length > 0) {
        for (let i = 0; i < element.children.length; i++) {
            deepCloneElement(element.children[i], options);
        }
    }
    return clone;
}
